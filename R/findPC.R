findPC<-function(sdev,method="All",aggregate=NULL,figure=FALSE){
  ...
  if(figure==TRUE){
    ggplot(eb,aes(x=x,y=sdev))+geom_point()+theme_bw()+
      ...
      geom_label(aes(x=dim_pie+3,y=6,label="Piecewise Linear Model"),size=5,col="red")+
      
      geom_vline(xintercept=dim_sed+0.1,lty=2,col="tan")+
      geom_label(aes(x=dim_sed+1,y=3.5,label="Second Derivative (LOESS)"),size=5,col="tan")+
      
      geom_vline(xintercept=dim_pr-0.1,lty=2,col="blue")+
      geom_label(aes(x=dim_pr+1.5,y=4.5,label="Preceding Residual"),size=5,col="blue")+
      ...
      geom_vline(xintercept=dim_clu,lty=2,col="green")+
      geom_label(aes(x=dim_clu+1,y=5,label="K-means Clustering"),size=5,col="green")+
      
      geom_vline(xintercept=dim_entr,lty=2,col="pink")+
      geom_label(aes(x=dim_entr+1,y=5,label="Information Dimension"),size=5,col="pink")
  }
}
